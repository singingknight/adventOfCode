...
class Day17 {
    ...
    async loadMap(pgm,input?:string[]) {
        this.brain.load(pgm);
        let line=[];
        if (input !== undefined) {
            this.brain.set(0,2);
            this.brain.setInput(input.reduce((p,l) => {
                p.push(...(l.split('').map(c => c.charCodeAt(0))));
                p.push(10);
                return p;
            },[]));
        }
...
describe("Day 17", () => {
    ...
    var testRunMap = async (pgm, expected) => {
        ...
        const target = new Day17();
        const result = await target.loadMap(pgm,input);
        expect(result).toEqual(expected);
    };
