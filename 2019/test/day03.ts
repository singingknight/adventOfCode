...
class Day03 {
    ...
    crosses = new Map<string, Position>();
    private formatPos(pos: Position) {
        return `${pos.x},${pos.y}`;
    }
    addCross(pos: Position) {
        this.crosses.set(this.formatPos(pos), { x: pos.x, y: pos.y });
    }
    markPos(pos: Position, n: number, prev: number[], stepCount: number) {
        if (prev === undefined) {
            prev = [n, undefined, undefined]
        }
        if (prev[n] === undefined) {
            prev[n] = stepCount;
        }
        this.board.set(this.formatPos(pos), prev);
    }
    getPos(pos: Position) {
        return this.board.get(this.formatPos(pos));
    }
    getSteps(pos: Position, n: number) {
        return this.board.get(this.formatPos(pos))[n];
    }
    wire(wire: string, n: number) {
        var pos = { x: 0, y: 0 };
        this.markPos(pos, 0, undefined, 0);
        const steps = wire.split(",");
        let stepCount = 0;
        steps.forEach(s => {
            const dir = s[0];
            const moves = Number(s.substr(1));
            for (let move = 0; move < moves; move++) {
                ...
                const prev = this.getPos(pos);
                if (prev !== undefined && prev[0] && prev[0] !== n) {
                    this.addCross(pos);
                }
                this.markPos(pos, n, prev, ++stepCount);
            }
        });
    }
    getNearest() {
        ...
        this.crosses.forEach(cross => {
            result = Math.min(result, Math.abs(cross.x) + Math.abs(cross.y))
        });
        return result;
    }
    getLeastSteps() {
        let result = 9999999999999;
        this.crosses.forEach(cross => {
            result = Math.min(result, this.getSteps(cross, 1) + this.getSteps(cross, 2))
        });
        return result;
    }
}
describe("Day 03", () => {
    var testNearest = (wires, expected) => {
        var target = new Day03();
        target.wire(wires[0], 1);
        target.wire(wires[1], 2);
        var result = target.getNearest();
        expect(result).toEqual(expected);
    }
    var testSteps = (wires, expected) => {
        var target = new Day03();
        target.wire(wires[0], 1);
        target.wire(wires[1], 2);
        var result = target.getLeastSteps();
        expect(result).toEqual(expected);
    }
    it("nearest", () => {
        testNearest(
