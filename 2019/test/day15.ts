...
class Day15 {
    ...
    mark(type, direction?) {
        ...
        if (direction !== undefined) {
            ...
            pos.y += this.directionOffsets[direction].y;
        }
        this.panels.set(this.getPos(pos), {
            pos: pos,
            type: type
        });
    }
    ...
    getMap(pgm) {
        return new Promise(resolve => {
            ...
            this.brain.setInputFunc(() => {
                ...
                return Promise.resolve(this.commands[this.nextMove]);
            });
            this.brain.setOutputFunc(n => {
                if (n !== 0) {
                    this.position.x += this.directionOffsets[this.nextMove].x;
                    this.position.y += this.directionOffsets[this.nextMove].y;
                }
                if (this.retrace) {
                    this.retrace = false;
    ...
    drawMap() {
        ...
        for (let y = topLeft.y; y <= bottomRight.y; y++) {
            ...
            for (let x = topLeft.x; x <= bottomRight.x; x++) {
                chars.push(this.read({ x: x, y: y }).type);
            }
    ...
    getMoves(x, y) {
        ...
        if (posData.moves === undefined) {
            posData.moves = 99999;
            this.panels.set(this.formatPoint(pos), posData);
            posData.moves =
                Math.min(
                    this.getMoves(x + 1, y),
                    this.getMoves(x - 1, y),
        ...
        return posData.moves;
    }
    getMinMoves() {
        return this.getMoves(0, 0);
    }
    max = 0;
    setDistance(x, y, moves) {
        ...
        if (posData.moves === undefined) {
            posData.moves = moves;
            if (moves > this.max) this.max = moves;
            this.panels.set(this.formatPoint(pos), posData);
            this.setDistance(x + 1, y, moves + 1)
            this.setDistance(x - 1, y, moves + 1)
            this.setDistance(x, y + 1, moves + 1);
            this.setDistance(x, y - 1, moves + 1);
        }
    }
    getMaxDistance() {
        this.panels.forEach((v,k) => {
            v.moves = undefined;
            this.panels.set(k,v);
        });
        this.setDistance(this.oxygen.x, this.oxygen.y, 0);
        return this.max;
    }
}
describe("Day 11", () => {
