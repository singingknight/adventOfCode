...
class Day17 {
    ...
    async runMap(pgm) {
        ...
        this.brain.setInputFunc(() => {
            ...
            if (nextChar === input[0].length) {
                console.log(nextChar, input[0]);
                input.shift();
                nextChar = 0;
                return Promise.resolve(10);
            }
            return Promise.resolve(input[0].charCodeAt(nextChar++));
        });
        await this.brain.run();
        return sum;
    }
    ...
    isScaffold(char) {
        return char === '#' || this.isVacuumRobot(char);
    }
    getAlignmentSum() {
        let sum=0;
        for (let y=1;y<this.map.length-1;y++) {
            for (let x=1;x<this.map[y].length-1;x++) {
                if (this.isScaffold(this.map[y][x])
                 && this.isScaffold(this.map[y-1][x])
                 && this.isScaffold(this.map[y-1][x])
                 && this.isScaffold(this.map[y][x-1])
                 && this.isScaffold(this.map[y][x+1])) {
                    sum += x*y;
                }
            }
