...
interface Panel { pos: Point, color: number }
class Day11 {
    brain = new Intcode();
        ...
        this.brain.setOutputFunc(n => {
            switch (this.cmdType) {
                case 0:
                    ...
                    break;
                case 1:
                    this.direction = (4 + this.direction + (n === 0 ? -1 : 1)) % 4;
    ...
    getPanels() {
        ...
        this.panels.forEach(p => {
            if (p.pos.x < topLeft.x)
                topLeft.x = p.pos.x;
            if (p.pos.y < topLeft.y)
                topLeft.y = p.pos.y;
            if (p.pos.x > bottomRight.x)
                bottomRight.x = p.pos.x;
            if (p.pos.y > bottomRight.y)
                bottomRight.y = p.pos.y;
        });
        const lines = [];
