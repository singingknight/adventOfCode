class Day06 {
    map : number[] = new Array(1_000_000);
    count = 0;
    constructor() {
        for(let i=0;i<1_000_000;i++)
            this.map[i]=0;
    }
    set(x: number, y:number, f: (number) => number) {
        const pos = x*1000+y;
        this.map[pos]=f(this.map[pos]);
    }
    process(s: string, f: (number) => number) {
        const corners = s.split("through");
        const start = corners[0].split(',').map(c=>Number(c));
        const end = corners[1].split(',').map(c=>Number(c));
        for (let x = start[0];x<=end[0];x++)
            for (let y = start[1];y<=end[1];y++)
                this.set(x,y,f);
    }
    runCommand(cmd: string) {
        if (cmd.substr(0,8) === "turn on ") {
            this.process(cmd.substr(8), _ => 1);
        }
        if (cmd.substr(0,7) === "toggle ") {
            this.process(cmd.substr(7), b => 1-b);
        }
        if (cmd.substr(0,9) === "turn off ") {
            this.process(cmd.substr(9), _ => 0);
        }
    }
    runCommands(cmds: string) {
        cmds.split('\n').map(c=>this.runCommand(c.trim()));
    }
    getCount() {
        return this.map.reduce((p,n)=>p+n,0);
    }
    runBrightCommand(cmd: string) {
        if (cmd.substr(0,8) === "turn on ") {
            this.process(cmd.substr(8), b => b+1);
        }
        if (cmd.substr(0,7) === "toggle ") {
            this.process(cmd.substr(7), b => b+2);
        }
        if (cmd.substr(0,9) === "turn off ") {
            this.process(cmd.substr(9), b => b==0?0:b-1);
        }
    }
    runBrightCommands(cmds: string) {
        cmds.split('\n').map(c=>this.runBrightCommand(c.trim()));
    }
}
describe("Day 06", () => {
    var test = (s, expected) => {
        var target = new Day06();
        target.runCommands(s);
        var result = target.getCount();
        expect(result).toEqual(expected);
    }
    var testBright = (s, expected) => {
        var target = new Day06();
        target.runBrightCommands(s);
        var result = target.getCount();
        expect(result).toEqual(expected);
    }
    it("boolean", () => {
        test("", 0);
        test("turn on 0,0 through 999,999", 1_000_000);
    ...
    it("bright", () => {
        testBright("",0);
        testBright(input06, 15343601);
    })
});

const input06 = `turn off 660,55 through 986,197
